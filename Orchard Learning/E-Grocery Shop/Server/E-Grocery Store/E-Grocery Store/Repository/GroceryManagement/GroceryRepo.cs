...
namespace E_Grocery_Store.Repository.GroceryManagement
{
    public class GroceryRepo : IGroceryRepo
    {
        ...
        public async Task<List<Grocery>> GetGroceries()
        {
            try
            {
                var result = await appDbContext.Groceries.Include(g => g.Category).Include(g => g.Vendor).Include(g => g.Status).Where(g => g.StatusId == 2).ToListAsync();
                if (result.Count == 0)
                {
        ...
        public async Task<List<Grocery>> GetAdminGroceries()
        {
            try
            {
                var result = await appDbContext.Groceries.Include(g => g.Category).Include(g => g.Vendor).Include(g => g.Status).OrderBy(g => g.StatusId).ToListAsync();
                if (result.Count == 0)
                {
                    throw new ResponseException("No Groceris available");
                }
        ...
        public async Task AdminRejectGrocery(int id)
        {
            try
            {
                var grocery = await appDbContext.Groceries.FirstOrDefaultAsync(g => g.Id == id);
        ...
        public async Task AdminAcceptGrocery(int id)
        {
            try
            {
                var grocery = await appDbContext.Groceries.FirstOrDefaultAsync(g => g.Id == id);
